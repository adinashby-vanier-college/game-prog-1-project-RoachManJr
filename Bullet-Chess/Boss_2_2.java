...
public class Boss_2_2 extends Actor
{
    ...
    private int speed = 2;  // Bishop moves slowly at this speed
    ...
    public void act()
    {
        // Move towards the current target (diagonal edge or center)
        moveToTarget();

        // If reached target, move to a random diagonal direction
        if (atTarget())
        {
            moveToRandomDiagonal();
        }
    }

    // Move the bishop towards the current target
    ...
    // Check if the Bishop has reached the target
    private boolean atTarget()
    {
        return (Math.abs(getX() - targetX) <= speed) && (Math.abs(getY() - targetY) <= speed);
    }

    // Move the Bishop to a random diagonal direction (either top-left, top-right, bottom-left, or bottom-right)
    private void moveToRandomDiagonal()
    {
        ...
        {
            // Randomly choose a diagonal direction
